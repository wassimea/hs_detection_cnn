...
cpdef np.ndarray get_channels(unsigned short [:,:] rawDepth, headpoint):
    ...
    #x = np.asarray(c2)
    #cv2.normalize(rawDepth,  rawDepth, 0, 255, cv2.NORM_MINMAX)
    normalized_depth = np.asarray(rawDepth).astype(np.ushort)
    cv2.normalize(normalized_depth,  normalized_depth, 0, 255, cv2.NORM_MINMAX)
    mod = cv2.merge([normalized_depth, np.asarray(c2) ,np.asarray(c3)])    
    return mod
