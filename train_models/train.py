...
def train(net_factory, prefix, end_epoch,display=200, base_lr=0.01):
    ...
    image_batch, label_batch, bbox_batch,landmark_batch,filename_batch = read_tfrecord_v2.read_multi_tfrecords(dataset_dirs,batch_sizes)        
    
    image_batch_val, label_batch_val, bbox_batch_val,landmark_batch_val,filename_batch_val = read_tfrecord_v2.read_multi_tfrecords(dataset_dirs_val,[100,100])  
    ...
    #define placeholders
    input_image = tf.placeholder(tf.float32, shape=[config.BATCH_SIZE, image_size, image_size, config.input_channels], name='input_image')
    label = tf.placeholder(tf.float32, shape=[config.BATCH_SIZE,2], name='label')
    bbox_target = tf.placeholder(tf.float32, shape=[config.BATCH_SIZE, 4], name='bbox_target')
    landmark_target = tf.placeholder(tf.float32,shape=[config.BATCH_SIZE,4],name='landmark_target')

    #class,regression
    # get initial losses
    cls_loss_op,bbox_loss_op,landmark_loss_op,L2_loss_op,accuracy_op,cls_prob_op = net_factory(input_image, label, bbox_target,landmark_target,training=True)
    ...
    with tf.Session(config=config1) as sess:
        ...
        logs_dir = "/home/wassimea/Desktop/wassimea/work/train_models/logs"
            ...
            for step in range(MAX_STEP):
                try:
                    ...
                    if coord.should_stop():
                        ...
                        break
                    image_batch_array, label_batch_array, bbox_batch_array,landmark_batch_array, filename_batch_array = sess.run([image_batch, label_batch, bbox_batch,landmark_batch, filename_batch])
                    ...
                    bbox_batch_array = bbox_batch_array[p]
                    landmark_batch_array = landmark_batch_array[p]
                    ...
                    if (step+1) % display == 0:
                        ...
                        image_batch_array_val, label_batch_array_val, bbox_batch_array_val,landmark_batch_array_val, filename_batch_array_val = sess.run([image_batch_val, label_batch_val, bbox_batch_val,landmark_batch_val, filename_batch_val])
                        #acc = accuracy(cls_pred, labels_batch)
                        #han
                        cls_loss, bbox_loss,landmark_loss,L2_loss,lr,acc,cls_prob = sess.run([cls_loss_op, bbox_loss_op,landmark_loss_op,L2_loss_op,lr_op,accuracy_op,cls_prob_op],
                                                                    feed_dict={input_image: image_batch_array_val, label: label_batch_array_val, bbox_target: bbox_batch_array_val, landmark_target: landmark_batch_array_val})                
                    ...
                    if i * config.BATCH_SIZE > num: 
                        ...
                        print("Finished epoch ------- Posacc: " + str(posacc) + "-----Negacc: " + str(negacc))
