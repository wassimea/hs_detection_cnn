...
class Detector(object):
    ...
    def __init__(self):#, data_size, batch_size, model_path):
        ...
        with tf.Graph().as_default():
            ...
            saver.restore(self.sess, '/home/wassimea/Desktop/wassimea/work/train_models/mn/3_mobilenet_mse/-640')
...
def evaluate_3c(pingit,images_folder):
    ...
    for image in images:
        ...
        for i in range(len(list_thresholds)):
            results = get_iou_totals_per_image(gtobjects, list_positive_props[i],images_folder + image,percentage)
    ...
    for gt in gt_objects:
        ...
        if(common_functions.check_box_boundaries(gtbox) == True):
            ...
            for prop in positive_props:
                ...
                if(common_functions.bb_intersection_over_boxA(gtbox,prop[0]) >= 0.5):
                    total_positives += 1
                    list_found_indices.append(positive_props.index(prop))
                    total_props_iou += common_functions.bb_intersection_over_union(prop[0],gtbox)
