...
def train_ONet(prefix, end_epoch, display, lr):
    ...
    elif(config.mtcnn_model == "mobilenet"):
...
if __name__ == '__main__':
    ...
    config = singleton.configuration._instance.config
    model_name = 'MTCNN'
    model_path = config.model_out + str(config.input_channels) + '_' + config.mtcnn_model + '_' + config.bbox_loss + '/'
    prefix = model_path
