...
def train(net_factory, prefix, end_epoch,display=200, base_lr=0.01):
    ...
    cls_loss_op,L2_loss_op,cls_prob_op = net_factory(input_image, label,training=False)
    ...
    with tf.Session(config=config1) as sess:
        ...
        sess.run(init)
        saver.restore(sess, '/home/wassimea/Desktop/wassimea/work/train_models/mn/3_mobilenet_mse/-4')
        ...
        try:
            for step in range(MAX_STEP):
                try:
                    ...
                    '''
                    summary = sess.run(summary_op, feed_dict={input_image: image_batch_array, label: label_batch_array})
                    z = (step+1) % display
                    if (step+1) % display == 0:
                        ...
                        print(image_batch_array_val.std(), image_batch_array_val.mean())
                        #acc = accuracy(cls_pred, labels_batch)
                        #han
                        cls_loss,L2_loss,cls_prob = sess.run([cls_loss_op, L2_loss_op,cls_prob_op],
                                                                    feed_dict={input_image: image_batch_array_val, label: label_batch_array_val})                
                        print("%s : Step: %d, cls loss: %4f,L2 loss: %4f" % (
                        datetime.now(), step+1, cls_loss, L2_loss))
