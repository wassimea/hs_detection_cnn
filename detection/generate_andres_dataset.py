            ...
            for image in files:
                if ".png" in image:
                    ...
                    if(os.path.exists(customer + "/" + image.replace(".png",".json"))):
                        ...
                        with open("/Data2TB/andres_dataset/annotated/temp/383/33273/332096.json") as f:
                            oldData = json.load(f)
                        #img = cv2.imread(customer + "/" + image,cv2.IMREAD_UNCHANGED)
                        img = cv2.imread("/Data2TB/andres_dataset/annotated/temp/383/33273/332096.png",cv2.IMREAD_UNCHANGED)
                        img[:, :, 3] = 255
                        img = img[:,:,:3]

                        imcpy = img.copy()

                        head_xmin = oldData["head"]["xmin"]
                        head_ymin = oldData["head"]["ymin"]
                        head_xmax = oldData["head"]["xmax"]
                        head_ymax = oldData["head"]["ymax"]

                        cv2.rectangle(imcpy,(head_xmin, head_ymin), (head_xmax, head_ymax),(0,0,255), 1)
                        cv2.imshow("cpy",imcpy)
                        cv2.waitKey()
