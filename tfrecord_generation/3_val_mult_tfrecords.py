...
def create_tf_example():
  count = 0
  counter = 0
  writer_pos = tf.python_io.TFRecordWriter("/Data2TB/chl_data/mod/val/records/pos.record")   #output file
  writer_neg = tf.python_io.TFRecordWriter("/Data2TB/chl_data/mod/val/records/neg.record")   #output file
  ...
  with open("/Data2TB/chl_data/mod/val/val_pos_48.json") as f:
    jsonroi = json.load(f)
  for i in range(0,len(jsonroi['frames'])):        #looping through JSON objects
    filename_only = jsonroi['frames'][i]['file']
    filename = "/Data2TB/chl_data/mod/val/pos_png_48/" + filename_only
    if(os.path.exists(filename) and filename not in included):
  ...
  with open("/Data2TB/chl_data/mod/val/val_neg_48.json") as f:
    jsonneg = json.load(f)
  ...
  for i in range(0,len(jsonneg['frames'])):        #looping through JSON objects
    ...
    filename = "/Data2TB/chl_data/mod/val/neg_png_48/" + filename_only
