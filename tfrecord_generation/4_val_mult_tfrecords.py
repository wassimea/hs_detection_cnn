...
def create_tf_example():
  count = 0
  counter = 0
  writer_pos = tf.python_io.TFRecordWriter("/Data2TB/chl_data/depth_normalized/val/records/pos.record")   #output file
  writer_neg = tf.python_io.TFRecordWriter("/Data2TB/chl_data/depth_normalized/val/records/neg.record")   #output file
  ...
  with open("/Data2TB/chl_data/rgb/val/val_pos_48.json") as f:
    jsonroi = json.load(f)
  for i in range(0,len(jsonroi['frames'])):        #looping through JSON objects
    filename_only = jsonroi['frames'][i]['file']
    filename_rgb = "/Data2TB/chl_data/rgb/val/pos_png_48/" + filename_only
    filename_d = "/Data2TB/chl_data/depth_normalized/val/pos_png_48/" + filename_only
    if(os.path.exists(filename_rgb) and filename_rgb not in included):
        included.append(filename_rgb)
        counter+=1
        ...
        with tf.gfile.GFile(filename_rgb, 'rb') as fid:
            encoded_rgb = fid.read()
        with tf.gfile.GFile(filename_d, 'rb') as fid2:
            encoded_d = fid2.read()
        encoded_jpg = b",".join([encoded_rgb,encoded_d])
        xminh = 0 
        xmaxh = 0 
        yminh = 0
        ymaxh = 0
        ...
        if(label == 1):
            label = [0,1]
            print(str(counter) + ": " + str(label))
            tf_example = tf.train.Example(features=tf.train.Features(feature={
            'image/filename': _bytes_feature(str.encode(filename_only)),
            'image/encoded': _bytes_feature(encoded_rgb),
            'image/encoded_d': _bytes_feature(encoded_d),
  ...
  with open("/Data2TB/chl_data/rgb/val/val_neg_48.json") as f:
    jsonneg = json.load(f)
  ...
  for i in range(0,len(jsonneg['frames'])):        #looping through JSON objects
    ...
    filename_rgb = "/Data2TB/chl_data/rgb/val/neg_png_48/" + filename_only
    filename_d = "/Data2TB/chl_data/depth_normalized/val/neg_png_48/" + filename_only
    if(os.path.exists(filename_rgb)):
        with tf.gfile.GFile(filename_rgb, 'rb') as fid:
            encoded_rgb = fid.read()
        with tf.gfile.GFile(filename_d, 'rb') as fid:
            encoded_d = fid.read()
        encoded_jpg = encoded_rgb + encoded_d
        y = len(encoded_jpg)
