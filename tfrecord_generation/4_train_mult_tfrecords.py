...
def create_tf_example():
  count = 0
  counter = 0
  writer_pos = tf.python_io.TFRecordWriter("/Data2TB/chl_data/depth_normalized/train/records/pos.record")   #output file
  writer_neg = tf.python_io.TFRecordWriter("/Data2TB/chl_data/depth_normalized/train/records/neg.record")   #output file
  ...
  with open("/Data2TB/chl_data/rgb/train/train_pos_augmented_48.json") as f:
    jsonroi = json.load(f)
  for i in range(0,len(jsonroi['frames'])):        #looping through JSON objects
    filename_only = jsonroi['frames'][i]['file']
    filename_rgb = "/Data2TB/chl_data/rgb/train/pos_png_augmented_48/" + filename_only
    filename_d = "/Data2TB/chl_data/depth_normalized/train/pos_png_augmented_48/" + filename_only
    if(os.path.exists(filename_rgb)):
        ...
        y = len(encoded_d)
        z = len(encoded_rgb)
        #decoded_image = tf.image.decode_image(encoded_d, channels=1)
        #sess = tf.Session()
  ...
  with open("/Data2TB/chl_data/rgb/train/train_neg_48.json") as f:
    jsonneg = json.load(f)
  ...
  for i in range(0,len(jsonneg['frames'])):        #looping through JSON objects
    ...
    filename_rgb = "/Data2TB/chl_data/rgb/train/neg_png_48/" + filename_only
    filename_d = "/Data2TB/chl_data/depth_normalized/train/neg_png_48/" + filename_only
    if(os.path.exists(filename_rgb)):
        with tf.gfile.GFile(filename_rgb, 'rb') as fid:
            encoded_rgb = fid.read()
        with tf.gfile.GFile(filename_d, 'rb') as fid:
